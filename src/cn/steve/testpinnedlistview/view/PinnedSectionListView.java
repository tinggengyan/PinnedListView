package cn.steve.testpinnedlistview.view;

import cn.steve.testpinnedlistview.BuildConfig;
...
public class PinnedSectionListView extends ListView {
  ...
  private final OnScrollListener mOnScrollListener = new OnScrollListener() {
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount,
      ...
      if (isFirstVisibleItemSection) {
        ...
        if (sectionView.getTop() == getPaddingTop()) { // view sticks to the top, no need for pinned
                                                       // shadow
          destroyPinnedShadow();
  ...
  public void initShadow(boolean visible) {
    if (visible) {
      if (mShadowDrawable == null) {
        mShadowDrawable =
            new GradientDrawable(Orientation.TOP_BOTTOM, new int[] {Color.parseColor("#ffa0a0a0"),
  ...
  void ensureShadowForPosition(int sectionPosition, int firstVisibleItem, int visibleItemCount) {
    ...
    if (nextPosition < getCount()) {
      int nextSectionPosition =
          findFirstVisibleSectionPosition(nextPosition, visibleItemCount
  ...
  int findFirstVisibleSectionPosition(int firstVisibleItem, int visibleItemCount) {
    ...
    if (getLastVisiblePosition() >= adapterDataCount) return -1; // dataset has changed, no
                                                                 // candidate

    if (firstVisibleItem + visibleItemCount >= adapterDataCount) {// added to prevent index
                                                                  // Outofbound (in case)
